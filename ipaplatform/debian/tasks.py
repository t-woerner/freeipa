...
""")
    ...
    def write_ca_certificates_dir(self, directory, ca_certs):
        ...
        for info in ca_certs:
            ...
            subject = DN(info.cert.subject)
            issuer = DN(info.cert.issuer)
            ...
            filename = (
                f'{subject.ldap_text()} {info.cert.serial_number}.crt'
            )
...
Description: {info.nickname}
Subject: {subject.ldap_text()}
Issuer: {issuer.ldap_text()}
Serial Number (dec): {info.cert.serial_number}
Serial Number (hex): {info.cert.serial_number:#x}
